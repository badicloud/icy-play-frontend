...
const pendingResetEmailKey = "icyplay.pendingResetEmail";
...
export default function ForgotPasswordPage() {
  ...
  useEffect(() => {
    // Warm the reCAPTCHA script so the first submit is not held up by it.
  ...
  async function handleSubmit(event: React.FormEvent) {
    ...
    try {
      ...
      sessionStorage.setItem(pendingResetEmailKey, email);
          ...
          {!isSent ? (
            <>
              ...
              <p className="mt-2 text-xl font-bold text-[#071955]">
                We&apos;ll email you a <span className="text-[#1264f7]">reset link</span>.
              </p>
              <div className="mt-5 h-1 w-14 rounded-full bg-[#1264f7]" />
              <p className="mt-5 max-w-lg text-lg leading-8 text-slate-600">
                Enter the email address on your IcyPlay account and we&apos;ll send you a link to
                choose a new password.
              </p>

              <form onSubmit={handleSubmit} className="mt-7 w-full">
                <div className="text-left">
                  <label htmlFor="email" className="block text-base font-extrabold text-[#071955]">
                    Email address
                  </label>
                  <input
                    id="email"
            ...
            <>
              ...
              <p className="mt-2 text-xl font-bold text-[#071955]">
                A <span className="text-[#1264f7]">reset link</span> is on its way.
              </p>
              <div className="mt-5 h-1 w-14 rounded-full bg-[#1264f7]" />
              <p className="mt-5 max-w-lg text-lg leading-8 text-slate-600">
                If an account exists for that address, you&apos;ll get an email shortly with a link
              ...
              <div className="mt-7 w-full rounded-2xl border border-slate-200/80 bg-white p-5 text-left shadow-[0_18px_55px_rgba(15,23,42,0.08)]">
                <InfoRow icon={<EmailOutlined fontSize="small" />} title="Check your inbox">
                  ...
                  <br />
                  <strong className="break-all font-bold text-[#1264f7]">{email}</strong>
                </InfoRow>
                ...
                <div className="border-t border-slate-100" />
                <InfoRow icon={<TimerOutlined fontSize="small" />} title="Link expires in 60 minutes">
                  For your security the link can only be used once, and it signs you out everywhere.
                </InfoRow>
              </div>
